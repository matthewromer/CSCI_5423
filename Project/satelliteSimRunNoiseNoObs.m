function [] = satelliteSimRunNoiseNoObs(A,B,C,K,L,Gamma,Deg,Lap,w,a,string)
...
[~,X] = ode45(@noisy_ODE_no_obs,T,x0,options,noise_hist,T,A,B,C,Gamma,Deg,K,L);
...
for ii = 1:length(X)
    ...
    for jj = 1:4    
        pos(:,ii,jj) = trans*[a+X(ii,(jj-1)*4+1);X(ii,(jj-1)*4+2)];
    end
end
